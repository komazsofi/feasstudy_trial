...
workingdirectory="O:/Nat_Sustain-proj/_user/ZsofiaKoma_au700510/FeasStudy/data/lidar/dhm2015/"
setwd(workingdirectory)

metricname=list.files(pattern = "*.tar.bz2")
metricnames=substr(metricname,1,nchar(metricname)-8)

#metricnames2=metricnames[metricnames != "canopy_height" & metricnames != "vegetation_density"]

for (j in metricnames) {
  print(j)
  
  filelist=list.files(path=paste0(workingdirectory,j,"/"),pattern = "*.tif",full.names = TRUE)
  
  metric=mosaicList(filelist)
  writeRaster(metric,paste("O:/Nat_Sustain-proj/_user/ZsofiaKoma_au700510/FeasStudy/data/lidar/dhm2015_processed/",j,".tif",sep=""),overwrite=TRUE)
